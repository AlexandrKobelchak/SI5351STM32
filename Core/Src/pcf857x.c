...
bool pcf857x_Read(uint8_t addr, uint8_t pin)
{
	if(_is8575)
	{
		...
		return _BIT_IS_SET(pcf857x_Read16(addr), pin);
	}
	else
	{
		if(pin > 7)
...
PCF857x_TypeDef pcf857x_Write(uint8_t addr, uint8_t pin, bool value)
{
	...
	if(_is8575)
	{
		if(pcf857x_Write16(addr, _bufferPins) != PCF857x_OK)
		{
			_error = PCF857x_I2C_ERROR;
			return PCF857x_I2C_ERROR;
		}
	...
	{
		if(pcf857x_Write8(addr, _bufferPins) != PCF857x_OK)
		{
			_error = PCF857x_I2C_ERROR;
			return PCF857x_I2C_ERROR;
		}
...
PCF857x_TypeDef pcf857x_RotateLeft(uint8_t addr, uint8_t n)
{
	if(_is8575)
	{
		if(pcf857x_RotateRight(addr, 16 - (n & 15)) != PCF857x_OK)
		{
			_error = PCF857x_I2C_ERROR;
			return PCF857x_I2C_ERROR;
		}
	...
	{
		if(pcf857x_RotateRight(addr, 8 - (n & 7)) != PCF857x_OK)
		{
			_error = PCF857x_I2C_ERROR;
			return PCF857x_I2C_ERROR;
		}
